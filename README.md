...
### دوربین‌های IP کافه که هنوز اضافه نشده‌اند
این پنج تا موقتاً از کانفیگ حذف شدند تا مسیر/کدک درستشان مشخص شود:

| دوربین | IP | مشکل | مسیری که باید تست شود |
|--------|-----|------|------------------------|
| parking3_90 | 192.168.51.90 | استریم بی‌اعتبار | `/stream1` |
| parking3_89 | 192.168.51.89 | timeout | `/stream1` |
| ashpazkhane_99 | 192.168.51.99 | H.265 چندلایه | دوربین را H.264 کن؛ `/Streaming/Channels/101` |
| rest_100 | 192.168.51.100 | timeout | `/Streaming/Channels/101` |
| rest_101 | 192.168.51.101 | timeout | `/stream1` |

بعد از پیدا کردن مسیر درست، طبق بخش «افزودن دوربین» به `config/cafe/config.yml`
اضافه‌شان کن.
