...
| گروه | مسیر | لاگین |
|---|---|---|
| همه بخش‌ها از پرتال (`:8888`) | nginx → پورت داخلی `5000` | **یک** کوکی پرتال (`portal_session`) |
| پورت مستقیم `897x` روی هاست | UI خود Frigate | لاگین جدا (دور زدن پرتال) |

پورت `5000` به هاست publish نمی‌شود. بدون کوکی پرتال، `/cafe/` و بقیه به فرم ورود پرتال برمی‌گردند؛ فرم Frigate دیده نمی‌شود.
